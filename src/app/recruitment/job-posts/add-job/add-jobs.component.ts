...
export class AddJobsComponent implements OnInit {
  ...
  addJob(addJobFrm: NgForm) {
    ...
    if (this.jobData.skill.length) {
      ...
      if (this.jobData.recruiters.length > 0) {
        this.recError = null;
        this.jobData.recruiters.forEach(type => {
          this.jobData.recruiter.userId = type.id;
        });
      }
      // else {
      //   this.loading = false;
      ...
      // this.http.post('http://service.tedpros.com/job/addJob', finalData, this.options)
